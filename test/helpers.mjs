...
export async function fixtureWorkflow(){return{benchmark:await read("workflow/benchmark.json"),prompt:await fs.readFile(`${ROOT}/workflow/prompt.md`,"utf8"),cases:await read("workflow/cases.json"),truth:await read("workflow/ground-truth.json"),schema:await read("workflow/output.schema.json"),models:await read("config/models.json"),pricing:await read("config/model-pricing.json"),sourceLedger:await read("workflow/source-ledger.json")};}
